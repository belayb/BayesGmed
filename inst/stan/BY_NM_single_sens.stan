...
model { 
  ...
  for (n in 1:N){
    ...
    ll_0 = log_inv_logit(eta_yu0[n]) * Y[n]+log1m_inv_logit(eta_yu0[n]) * (1 - Y[n])+
           normal_lpdf(M[n]|eta_mu0[n],scale_sd_m)+log1m_inv_logit(eta_u[n]);
    
    //contribution if U = 0

    ll_1 = log_inv_logit(eta_yu0[n] + alphaU) * Y[n]+log1m_inv_logit(eta_yu0[n] + alphaU) * (1 - Y[n])+
           normal_lpdf(M[n]|eta_mu0[n] + betaU,scale_sd_m)+log1m_inv_logit(eta_u[n]);
...
generated quantities { 
  ...
  for (n in 1:N) { 
    ...
    // sample Ma where a = 0 
    M_a0[n] = normal_rng (X[row_i] * betaZ + U[n] * betaU, scale_sd_m);
    // sample Ma where a = 1 
